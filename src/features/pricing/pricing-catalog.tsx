...
export function PricingCatalog() {
  ...
  return <>
    ...
    <div className="pricing-grid pricing-page-grid">{plans.map((plan) => <article className={`price-card${selected === plan.id ? " price-card-featured" : ""}`} key={plan.id} onClick={() => setSelected(plan.id)}><span className="plan-kicker">{plan.kicker}</span><h3>{plan.name}</h3><div className="price-placeholder"><strong>Условия в личном кабинете</strong><small>cabinet.stvillage.ru</small></div><p>{plan.description}</p><ul className="plan-list"><li>Выбранное число устройств: {devices}</li><li>Управление в отдельном кабинете</li><li>Инструкции для основных платформ</li></ul><a className={selected === plan.id ? "button button-primary" : "button button-secondary"} href={CABINET_URL} target="_blank" rel="noreferrer">Открыть кабинет <span>↗</span></a></article>)}</div>
