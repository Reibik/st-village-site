...
if [[ "$target_sha" == "$active_sha" ]] && systemctl is-active --quiet st-village-dev-site.service; then
  exit 0
fi
...
set -a
source /etc/st-village/dev.env
set +a
export CI=true
export NEXT_PUBLIC_SITE_URL="https://dev.stvillage.ru"
...
# Preserve the pre-persistence review queue from the service's private /var/tmp
# before the first restart that uses the durable data directory.
if [[ ! -s "$data_file" ]]; then
  service_pid="$(systemctl show -p MainPID --value st-village-dev-site.service 2>/dev/null || true)"
...
if ! systemctl restart st-village-dev-site.service; then
  if [[ -n "$previous" && -d "$previous" ]]; then
    ln -sfn "$previous" "${app_root}/current.next"
    mv -Tf "${app_root}/current.next" "${app_root}/current"
    systemctl restart st-village-dev-site.service || true
  fi
  exit 1
fi
...
healthy=0
for _ in {1..30}; do
  if curl --fail --silent --show-error --max-time 3 http://127.0.0.1:3001/api/health >/dev/null; then
    healthy=1
    break
  fi
  sleep 1
